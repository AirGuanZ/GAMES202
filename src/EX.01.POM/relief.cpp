...
void ReliefMappingRenderer::initialize()
{
    shader_.initializeStageFromFile<VS>(
        "./asset/EX.01/relief.hlsl", nullptr, "VSMain");
    shader_.initializeStageFromFile<PS>(
        "./asset/EX.01/relief.hlsl", nullptr, "PSMain");
...
void ReliefMappingRenderer::setCamera(const Float3 &eye, const Mat4 &viewProj)
{
    psPerFrameData_.eye = eye;
    viewProj_           = viewProj;
}

void ReliefMappingRenderer::setLight(const DirectionalLight &light)
{
    psPerFrameData_.light = light;
}

void ReliefMappingRenderer::setTracer(
    float linearStep, int maxLinearStepCount, int binaryStepCount)
{
    psPerFrameData_.linearStep         = linearStep;
    psPerFrameData_.maxLinearStepCount = maxLinearStepCount;
    psPerFrameData_.binaryStepCount    = binaryStepCount;
}

void ReliefMappingRenderer::setDiffStep(int step)
{
    psPerFrameData_.diffStep = static_cast<float>(step);
}

void ReliefMappingRenderer::begin()
{
    shader_.bind();
    shaderRscs_.bind();
...
void ReliefMappingRenderer::end()
{
...
void ReliefMappingRenderer::render(const Mesh &mesh)
{
